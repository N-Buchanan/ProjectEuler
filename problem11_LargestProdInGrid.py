...
def calculateVerticalProd(r,c):
	prod = 1
	if r < ROW - 4:
		for i in range(4):
			prod = prod * numbers[r+i][c]
	else:
		prod = -1
	return prod

def calculateHorizontalProd(r,c):
	prod = 1
	if c < COL - 4:
		for i in range(4):
			prod = prod * numbers[r][c+i]
	else:
		prod = -1
	return prod

def calculateRightDiagonalProd(r,c):
	prod = 1
	if r < ROW -4 and c < COL - 4:
		for i in range(4):
			prod = prod * numbers[r+i][c+i]
	else:
		prod = -1
	return prod

def calculateLeftDiagonalProd(r,c):
	prod = 1
	if r >= 3 and c < COL -4:
		for i in range(4):
			prod = prod * numbers[r-i][c+i]
...
products = [vprod,hprod,rdprod,ldprod,maxp]

for i in range(20):
	for j in range(20):
		products[vprod] = calculateVerticalProd(i,j)
		products[hprod] = calculateHorizontalProd(i,j)
		products[rdprod] = calculateRightDiagonalProd(i,j)
		products[ldprod] = calculateLeftDiagonalProd(i,j)
		products[maxp] = maxProduct
